...
async function resolveVoucherScopeForAdmin(voucherId) {
    if (!voucherId || !mongoose_1.default.isValidObjectId(voucherId))
        return null;
    const voucherDoc = await Voucher_1.default.findById(voucherId)
        .select("_id templateId")
        .lean();
    if (!voucherDoc) {
        return new mongoose_1.default.Types.ObjectId(voucherId);
    }
    return (voucherDoc.templateId ||
        voucherDoc._id);
}
async function getVoucherCountForTemplate(userId, voucherId) {
    if (!voucherId) {
        ...
        return Math.max(0, physicalCount + Number(user?.voucherBalance || 0));
    }
    const [physicalCount, user] = await Promise.all([
        Voucher_1.default.countDocuments({
            userId: new mongoose_1.default.Types.ObjectId(userId),
...
async function reconcileTransferUnlocksForUserFromAdmin(userId, voucherId) {
    const now = new Date();
    const query = {
    ...
    for (const transfer of transfers) {
        const currentVoucherCount = await getVoucherCountForTemplate(userId, transfer.voucherId);
...
router.put("/users/:userId/update-vouchers", adminAuth, async (req, res) => {
    try {
        const { userId } = req.params;
        const { voucherBalance, reason, voucherId } = req.body;
        if (voucherBalance === undefined ||
            voucherBalance === null ||
        ...
        if (voucherId) {
            ...
            user.markModified("voucherBalances");
        }
        else {
            oldBalance = user.voucherBalance || 0;
            user.set({ voucherBalance: newBalance });
        }
        const diff = newBalance - oldBalance;
        await user.save();
        const voucherObjectId = voucherId && mongoose_1.default.isValidObjectId(voucherId)
            ? new mongoose_1.default.Types.ObjectId(String(voucherId))
            : undefined;
        await reconcileTransferUnlocksForUserFromAdmin(String(user._id), voucherObjectId);
        try {
            const io = global.io;
            if (io) {
        ...
        res
            .status(500)
            .json({ success: false, message: "Failed to update voucher balance" });
    }
});
// GET /api/admin/users/by-phone/:phone — Look up a user by phone number
router.get("/users/by-phone/:phone", adminAuth, async (req, res) => {
    try {
        ...
        const user = await User_1.default.findOne({ phone: { $in: patterns } }).select("name phone credits voucherBalance isVoucherAdmin specialCredits createdAt");
        if (!user) {
            return res
                .status(404)
                .json({ success: false, message: "User not found" });
...
// GET /api/admin/users/:userId/voucher-stats?voucherId=xxx
// Returns a user's per-voucher MLM credits, slot counts, and voucher balance
router.get("/users/:userId/voucher-stats", adminAuth, async (req, res) => {
    try {
        const { userId } = req.params;
        const { voucherId } = req.query;
        const user = await User_1.default.findById(userId)
            .select("name phone credits voucherBalance voucherBalances isVoucherAdmin specialCredits level")
            .lean();
        if (!user) {
            return res
                .status(404)
        ...
        // Fetch SpecialCredit slots for this user + voucher
        const slotQuery = { ownerId: userId };
        if (voucherId) {
            try {
                slotQuery.voucherId = new mongoose_1.default.Types.ObjectId(voucherId);
            }
            catch (_) { }
        }
        const slots = await SpecialCredit_1.default.find(slotQuery).lean();
        const availableSlots = slots.filter((s) => s.status === "available");
        const sentSlots = slots.filter((s) => s.status === "sent");
        const availableCredits = availableSlots.reduce((sum, s) => sum + (s.creditAmount || 0), 0);
        const distributedCredits = sentSlots.reduce((sum, s) => sum + (s.creditAmount || 0), 0);
        const uniqueRecipients = new Set(sentSlots.map((s) => s.recipientId?.toString()).filter(Boolean)).size;
        res.json({
            success: true,
            stats: {
                availableCredits,
                ...
                voucherBalance: (() => {
                    ...
                    return user.voucherBalance || 0;
                })(),
                generalCredits: user.credits || 0,
                isVoucherAdmin: user.isVoucherAdmin || false,
            },
        });
    }
    catch (error) {
        console.error("ADMIN USER VOUCHER STATS ERROR", error);
...
router.post("/vouchers", adminAuth, async (req, res) => {
    try {
        const { companyLogo, companyName, title, phoneNumber, address, amount, discountPercentage, validity, voucherImage, description, expiryDate, isPublished = false, minVouchersRequired, } = req.body;
        ...
        const templateObjectId = new mongoose_1.default.Types.ObjectId();
        const voucher = await Voucher_1.default.create({
            ...
            minVouchersRequired: minVouchersRequired !== undefined
    ...
    try {
        ...
        res.json({
            ...
            vouchers: vouchers.map((v) => ({
                ...
                minVouchersRequired: v.minVouchersRequired ?? 5,
...
router.post("/vouchers/:id/publish", adminAuth, async (req, res) => {
    try {
        ...
        if (userIds && Array.isArray(userIds) && userIds.length > 0) {
            const templateId = voucherTemplate.templateId || voucherTemplate._id;
            const validUserIds = userIds
                .filter((uid) => mongoose_1.default.isValidObjectId(uid))
                .map((uid) => new mongoose_1.default.Types.ObjectId(uid));
            // Avoid duplicate voucher copies on repeated publish actions.
            const existingAssigned = await Voucher_1.default.find({
                templateId,
                userId: { $in: validUserIds },
            })
                .select("userId")
                .lean();
            const existingSet = new Set(existingAssigned
                .map((v) => v.userId?.toString())
                .filter(Boolean));
            const toAssign = validUserIds.filter((uid) => !existingSet.has(uid.toString()));
 ...
 */
router.post("/vouchers/:id/unpublish", adminAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const voucher = await Voucher_1.default.findByIdAndUpdate(id, { isPublished: false, publishedAt: null }, { new: true });
        if (!voucher)
            return res
                .status(404)
                .json({ success: false, message: "Voucher not found" });
        const templateId = voucher.templateId || voucher._id;
        await Voucher_1.default.updateMany({
            templateId,
        ...
        res.json({
            ...
            voucher: { id: voucher._id, isPublished: false },
        });
    }
    catch (error) {
        res
            .status(500)
            .json({ success: false, message: "Failed to unpublish voucher" });
    }
});
/**
...
router.delete("/vouchers/:id", adminAuth, async (req, res) => {
    try {
        ...
        const voucher = await Voucher_1.default.findById(id).select("_id templateId").lean();
        if (!voucher) {
            return res.status(404).json({
                success: false,
                message: "Voucher not found",
        ...
        const templateId = voucher.templateId ||
            voucher._id;
        const deleteQuery = {
            $or: [
                { _id: templateId },
        ...
        const [deleteResult, transferLogResult, specialCreditResult] = await Promise.all([
            Voucher_1.default.deleteMany(deleteQuery),
            VoucherTransferLog_1.default.deleteMany({ voucherId: templateId }),
            SpecialCredit_1.default.deleteMany({ voucherId: templateId }),
        ]);
        res.json({
            success: true,
...
router.get("/mlm/slots", adminAuth, async (req, res) => {
    try {
        const { voucherId, userId } = req.query;
        console.log("📊 ADMIN MLM SLOTS - Request received", { voucherId, userId });
        if (voucherId && !mongoose_1.default.isValidObjectId(voucherId)) {
            return res
                .status(400)
        ...
        let targetUser = userId
            ? await User_1.default.findById(userId)
                ...
                .lean()
            : await User_1.default.findOne({ isVoucherAdmin: true })
                .select("name phone level isVoucherAdmin specialCredits")
                .lean();
        console.log("👤 ADMIN MLM SLOTS - Target user", {
        ...
        if (!targetUser) {
            return res.json({
                ...
                summary: {
                    ...
                    availableSlots: 0,
                    usedSlots: 0,
                    creditPerSlot: SPECIAL_CREDIT_CHAIN_ADMIN[0],
                },
        ...
        if (voucherId) {
            // prefer raw ObjectId when valid so admin views are scoped per-voucher
            // and don't accidentally collapse multiple vouchers under a template.
            let rawVoucherId = null;
            if (mongoose_1.default.isValidObjectId(voucherId)) {
                rawVoucherId = new mongoose_1.default.Types.ObjectId(voucherId);
            }
            if (rawVoucherId) {
                query.voucherId = rawVoucherId;
            }
            else {
                const canonicalVoucherId = await resolveVoucherScopeForAdmin(voucherId);
                if (!canonicalVoucherId) {
                    return res
                        .status(404)
                        .json({ success: false, message: "Voucher not found" });
                }
                query.voucherId = canonicalVoucherId;
            }
        }
        console.log("🔎 ADMIN MLM SLOTS - Query for slots", {
            query: JSON.stringify(query),
        });
        const slots = await SpecialCredit_1.default.find(query)
            .populate("recipientId", "name phone")
            .sort({ slotNumber: 1 })
            .lean();
        console.log("📦 ADMIN MLM SLOTS - Slots retrieved", {
            count: slots.length,
            slotNumbers: slots.map((s) => s.slotNumber),
        });
        const creditPerSlot = SPECIAL_CREDIT_CHAIN_ADMIN[targetUser.level || 0] ||
        ...
        console.log("✅ ADMIN MLM SLOTS - Success", {
            userId: targetUser._id.toString(),
        ...
        console.error("❌ ADMIN MLM SLOTS ERROR", {
            voucherId: req.query.voucherId,
...
router.post("/mlm/slots/initialize", adminAuth, async (req, res) => {
    try {
        const { adminUserId, voucherId, slotCount: rawSlotCount, creditAmount: customCreditAmount, } = req.body;
        if (!adminUserId || !voucherId)
            return res.status(400).json({
                success: false,
                message: "adminUserId and voucherId are required",
            });
        // the voucherId coming from the client might already be a bare ObjectId
        // which we should respect; otherwise fall back to the resolver logic.
        let canonicalVoucherId = null;
        if (!mongoose_1.default.isValidObjectId(voucherId)) {
            // not an ObjectId string -> lookup via resolver
            canonicalVoucherId = await resolveVoucherScopeForAdmin(voucherId);
        }
        else {
            canonicalVoucherId = new mongoose_1.default.Types.ObjectId(voucherId);
        }
        if (!canonicalVoucherId) {
            return res
                .status(404)
        ...
        const voucher = await Voucher_1.default.findById(voucherId).lean();
        if (!voucher)
            return res
                .status(404)
                .json({ success: false, message: "Voucher not found" });
        ...
        await admin.save();
        const existingCount = await SpecialCredit_1.default.countDocuments({
            ownerId: adminUserId,
        ...
        if (existingCount >= slotCount) {
            return res.json({
                success: true,
                message: `Admin already has ${existingCount} slots for ${voucher.companyName}`,
                slots: existingCount,
            });
        }
        // Use custom credit amount if provided, otherwise fall back to chain default
        const creditAmount = customCreditAmount && Number(customCreditAmount) > 0
        ...
        await SpecialCredit_1.default.insertMany(toCreate);
        res.json({
            success: true,
            message: `${toCreate.length} slots initialized for ${voucher.companyName}`,
...
router.post("/mlm/slots/increase", adminAuth, async (req, res) => {
    try {
        const { voucherId, count, adminUserId, creditAmount: customCreditAmount, } = req.body;
        ...
        const lastSlot = await SpecialCredit_1.default.findOne({
            ownerId: admin._id,
            voucherId: canonicalVoucherId,
        })
            .sort({ slotNumber: -1 })
            .lean();
        const startFrom = lastSlot ? lastSlot.slotNumber + 1 : 1;
        // Use custom credit amount if provided, otherwise fall back to chain default
        const creditAmount = customCreditAmount && Number(customCreditAmount) > 0
            ? Number(customCreditAmount)
        ...
        await SpecialCredit_1.default.insertMany(newSlots);
        await User_1.default.findByIdAndUpdate(admin._id, {
            $inc: { "specialCredits.availableSlots": count },
        });
        res.json({
            success: true,
            message: `${count} slot(s) added for ${voucher.companyName}`,
            slotsAdded: count,
            totalSlots: startFrom - 1 + count,
            creditPerSlot: creditAmount,
        });
    }
    catch (error) {
...
router.delete("/mlm/slots", adminAuth, async (req, res) => {
    try {
        const { voucherId, slotNumbers, deleteAll, userId } = req.body;
        if (!voucherId)
            return res
                .status(400)
        ...
        if (userId) {
            ...
            const adminUser = await User_1.default.findOne({ isVoucherAdmin: true })
                .select("_id")
                .lean();
        ...
        if (!deleteAll)
            query.slotNumber = { $in: slotNumbers };
        const result = await SpecialCredit_1.default.deleteMany(query);
        ...
        await User_1.default.findByIdAndUpdate(ownerId, {
