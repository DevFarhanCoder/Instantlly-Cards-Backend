...
async function createRazorpayOrder(params) {
    ...
    const response = await axios_1.default.post("https://api.razorpay.com/v1/orders", payload, {
        headers: {
...
function verifyRazorpaySignature(params) {
    ...
    const expectedSignature = crypto_1.default
        ...
        .update(payload)
        .digest("hex");
    return crypto_1.default.timingSafeEqual(Buffer.from(expectedSignature), Buffer.from(params.razorpaySignature));
}
async function activateOrderAndPromotion(params) {
    const now = new Date();
    const expiryDate = new Date(now.getTime() + params.order.durationDays * 24 * 60 * 60 * 1000);
...
router.post("/seed-default", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const operations = DEFAULT_PRICING_ROWS.flatMap((row) => AREA_TYPES.map((areaType) => {
        ...
        if (operations.length > 0) {
            await PromotionPricingPlan_1.default.bulkWrite(operations);
        }
        return res.json({
            success: true,
...
router.get("/catalog", async (_req, res) => {
    try {
        const plans = await PromotionPricingPlan_1.default.find({ isActive: true })
            .select("-__v")
...
router.get("/quote", async (req, res) => {
    try {
        ...
        const plan = await PromotionPricingPlan_1.default.findOne({
            areaType: area,
            rank: parsedRank,
...
router.post("/orders", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const { businessPromotionId, areaType, rank, durationDays = 30, paymentProvider = "razorpay", } = req.body;
        if (!businessPromotionId || !mongoose_1.default.Types.ObjectId.isValid(businessPromotionId)) {
            return res.status(400).json({
                success: false,
        ...
        if (!Number.isInteger(rank) || rank < 1 || rank > 21) {
            return res.status(400).json({
                success: false,
                message: "Invalid rank. Allowed range is 1 to 21",
            });
        ...
        if (promotion.listingIntent !== "promoted") {
            return res.status(400).json({
                success: false,
                message: "Listing is not marked as premium. Please select premium listing type first.",
            });
        }
        const pricingPlan = await PromotionPricingPlan_1.default.findOne({
            areaType,
            rank,
            durationDays,
            isActive: true,
        ...
        const order = await PromotionOrder_1.default.create({
            userId,
            businessPromotionId,
...
router.post("/orders/:orderId/verify-payment", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const order = await PromotionOrder_1.default.findOne({ _id: orderId, userId });
        if (!order) {
...
// User: get own orders
router.get("/orders/me", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        const orders = await PromotionOrder_1.default.find({ userId })
...
router.post("/orders/:orderId/confirm-payment", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        const { orderId } = req.params;
        const { paymentId, paymentOrderId, metadata } = req.body;
        if (!mongoose_1.default.Types.ObjectId.isValid(orderId)) {
            return res.status(400).json({
                success: false,
        ...
        if (order.paymentProvider === "razorpay") {
            return res.status(400).json({
                success: false,
                message: "Use /orders/:orderId/verify-payment for Razorpay orders",
            });
        }
        const promotion = await BusinessPromotion_1.default.findOne({
            _id: order.businessPromotionId,
