...
const TempMessage_1 = __importDefault(require("../models/TempMessage"));
...
router.post('/send', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        try {
            await TempMessage_1.default.create({
                senderId,
                receiverId,
                text: text.trim(),
                messageId,
        ...
        if (receiver.pushToken && receiver.pushToken !== 'expo-go-local-mode') {
            try {
                ...
                await (0, pushNotifications_1.sendIndividualMessageNotification)(receiver.pushToken, sender.name, text, senderId);
...
router.post('/send-group', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (!group.members.includes(new mongoose_1.default.Types.ObjectId(senderId))) {
            ...
            for (const memberId of group.members) {
                ...
                if (memberIdStr !== senderId) { // Don't send to sender
                    ...
                    const memberSockets = Array.from(socketIO.sockets.sockets.values())
                        .filter((socket) => socket.userId === memberIdStr);
        ...
        try {
            ...
            const otherMembers = group.members.filter((id) => id.toString() !== senderId);
            ...
            const membersWithTokens = await User_1.default.find({
                _id: { $in: otherMembers },
            ...
            if (membersWithTokens.length > 0) {
                for (const member of membersWithTokens) {
                    try {
                        ...
                        await (0, pushNotifications_1.sendGroupMessageNotification)(member.pushToken, group.name, sender.name, text, groupId);
...
router.get('/groupmessage/:groupId', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const group = await mongoose_1.default.model('Group').findById(groupId);
...
router.post('/check-online-status', auth_1.requireAuth, async (req, res) => {
    try {
...
router.post('/typing-status/:userId', auth_1.requireAuth, async (req, res) => {
    try {
...
router.get('/typing-status/:userId', auth_1.requireAuth, async (req, res) => {
    try {
        const { userId } = req.params;
        const currentUserId = req.userId;
        if (!currentUserId) {
...
router.post('/group-typing-status/:groupId', auth_1.requireAuth, async (req, res) => {
    try {
        const { groupId } = req.params;
        const { isTyping } = req.body;
        const currentUserId = req.userId;
...
router.get('/group-typing-status/:groupId', auth_1.requireAuth, async (req, res) => {
    try {
        const { groupId } = req.params;
        const currentUserId = req.userId;
        if (!currentUserId) {
...
router.get('/check-all-pending', auth_1.requireAuth, async (req, res) => {
    try {
        const currentUserId = req.userId;
        if (!currentUserId) {
        ...
        const pendingMessages = await TempMessage_1.default.find({
        ...
        if (pendingMessages.length > 0) {
            console.log(`📬 ${pendingMessages.length} pending messages for user ${currentUserId}`);
        }
        res.status(200).json({
            success: true,
...
router.get('/check-pending/:userId', auth_1.requireAuth, async (req, res) => {
    try {
        const currentUserId = req.userId;
        ...
        const messages = await TempMessage_1.default.find({
...
router.post('/mark-delivered', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        // Mark messages as delivered
        const result = await TempMessage_1.default.updateMany({
            receiverId: currentUserId,
...
router.post('/mark-read', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const result = await mongoose_1.default.model('Message').updateMany({
            _id: { $in: messageIds },
...
router.delete('/cleanup-expired', auth_1.requireAuth, async (req, res) => {
    try {
        const now = new Date();
        const result = await TempMessage_1.default.deleteMany({
            expiresAt: { $lt: now }
        });
...
router.delete('/typing-status/clear', auth_1.requireAuth, async (req, res) => {
    try {
        const currentUserId = req.userId;
        if (!currentUserId) {
            return res.status(401).json({ error: 'Unauthorized' });
...
router.get('/check-group-pending/:groupId', auth_1.requireAuth, async (req, res) => {
    try {
        const currentUserId = req.userId;
        const { groupId } = req.params;
        if (!currentUserId) {
...
router.post('/mark-group-delivered', auth_1.requireAuth, async (req, res) => {
    try {
