...
router.get("/", async (req, res) => {
    try {
        ...
        const [listings, totalAgg] = await Promise.all([
            BusinessPromotion_1.default.aggregate([...basePipeline, { $skip: skip }, { $limit: limitNum }]),
            BusinessPromotion_1.default.aggregate([{ $match: matchStage }, { $count: "total" }]),
        ]);
        const total = totalAgg[0]?.total || 0;
        const promotedCount = listings.filter((item) => item.listingType === "promoted").length;
        const freeCount = listings.filter((item) => item.listingType === "free").length;
        res.json({
            success: true,
            data: listings,
            meta: {
...
router.get("/:id", async (req, res) => {
    try {
        ...
        await BusinessPromotion_1.default.findByIdAndUpdate(req.params.id, {
