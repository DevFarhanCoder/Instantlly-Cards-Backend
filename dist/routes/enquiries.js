 ...
 */
router.post("/:businessId/enquiry", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const userEnquiresWithinDay = await Enquiry_1.default.countDocuments({
            userId,
 ...
 */
router.get("/:businessId/enquiries", auth_1.requireAuth, async (req, res) => {
    try {
        const { businessId } = req.params;
        ...
        const total = await Enquiry_1.default.countDocuments(filter);
        // Calculate stats
        const allEnquiries = await Enquiry_1.default.find({ businessId });
        const stats = {
 ...
 */
router.post("/:enquiryId/respond", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const enquiry = await Enquiry_1.default.findById(enquiryId);
        if (!enquiry) {
            return res.status(404).json({
                success: false,
 ...
 */
router.patch("/:enquiryId/status", auth_1.requireAuth, async (req, res) => {
    try {
        const { enquiryId } = req.params;
 ...
 */
router.get("/:businessId/enquiry-stats", auth_1.requireAuth, async (req, res) => {
    try {
        const { businessId } = req.params;
        // Verify ownership
        const business = await BusinessPromotion_1.default.findOne({
        ...
        const allEnquiries = await Enquiry_1.default.find({ businessId });
        const thisMonth = allEnquiries.filter(e => new Date(e.createdAt) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000));
        const responded = allEnquiries.filter(e => e.lastResponseAt);
 ...
 */
router.get("/:businessId/combined-feedback", auth_1.requireAuth, async (req, res) => {
    try {
        const { businessId } = req.params;
        ...
        if (type === 'all' || type === 'enquiries') {
            const enquiries = await Enquiry_1.default.find({ businessId })
                .sort(sort === 'latest' ? { createdAt: -1 } : { createdAt: 1 })
                .limit(limitNum);
            enquiries.forEach(e => {
                feedback.push({
