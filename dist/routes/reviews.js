...
const reviewImageUpload = (0, multer_1.default)({
 ...
 */
router.post("/:businessId/reviews", auth_1.requireAuth, reviewImageUpload.array("photos", 5), async (req, res) => {
    try {
        ...
        const business = await BusinessPromotion_1.default.findById(businessId);
        if (!business) {
        ...
        await BusinessPromotion_1.default.findByIdAndUpdate(businessId, {
            $inc: {
 ...
 */
router.post("/:businessId/upload-photos", auth_1.requireAuth, reviewImageUpload.array("photos", 5), async (req, res) => {
    try {
        const { businessId } = req.params;
        const userId = req.userId;
        const files = req.files;
...
router.get("/:businessId/reviews", async (req, res) => {
    try {
        ...
        const totalReviews = await Review_1.default.countDocuments(filter);
        // Calculate stats
        const allReviews = await Review_1.default.find({
            businessId: businessId,
            isApproved: true,
            isSpam: false
        });
...
router.put("/:reviewId", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        await review.save();
        const updatedReview = await Review_1.default.findById(reviewId).populate('userId', '_id name profile picture');
    ...
    try {
        ...
        if (review.photos?.length) {
            const deleteOps = review.photos
                .map((photo) => photo?.cloudinary_id)
                .filter((key) => typeof key === "string" && key.length > 0)
                .map((Key) => s3Instance.deleteObject({
                Bucket: process.env.S3_BUCKET,
                Key
            }).promise());
 ...
 */
router.post("/:reviewId/reply", auth_1.requireAuth, async (req, res) => {
    try {
        const { reviewId } = req.params;
        ...
        const review = await Review_1.default.findById(reviewId).populate('businessId');
        if (!review) {
            return res.status(404).json({
                success: false,
 ...
 */
router.get("/:businessId/review-stats", auth_1.requireAuth, async (req, res) => {
    try {
        const { businessId } = req.params;
        const userId = req.userId;
        // Verify ownership
        ...
        const thisMonth = allReviews.filter(r => new Date(r.createdAt) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000));
        const stats = {
            totalReviews: allReviews.length,
            averageRating: allReviews.length > 0
            ...
            topSuggestions: (() => {
                ...
                allReviews.forEach(r => {
                    r.selectedSuggestions?.forEach((s) => {
 ...
 */
router.get("/admin/moderate", adminAuth_1.requireAdminAuth, async (req, res) => {
    try {
        ...
        if (isSpam === 'true') {
            filter.spamReports = { $size: parseInt(spamCount) || 1 };
        }
        const pageNum = Math.max(1, parseInt(page) || 1);
 ...
 */
router.patch("/:reviewId/moderate", adminAuth_1.requireAdminAuth, async (req, res) => {
    try {
        const { reviewId } = req.params;
 ...
 */
router.get("/admin/analytics", adminAuth_1.requireAdminAuth, async (req, res) => {
    try {
        const allReviews = await Review_1.default.find({ isApproved: true, isSpam: false });
        const spamReviews = await Review_1.default.find({ isSpam: true });
        const thisMonth = allReviews.filter(r => new Date(r.createdAt) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000));
        // Get top businesses
        const topBusinesses = await Review_1.default.aggregate([
        ...
        const analytics = {
            totalReviews: allReviews.length,
            totalBusinesses: await BusinessPromotion_1.default.countDocuments(),
            averageRating: allReviews.length > 0
                ? (allReviews.reduce((sum, r) => sum + r.rating, 0) / allReviews.length).toFixed(1)
                : 0,
