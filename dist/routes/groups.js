    ...
    try {
        ...
        const groupsWithTransferInfo = groups.map(group => {
            ...
            if (groupObj.admin?._id &&
                ...
                const previousAdmin = groupObj.adminTransferInfo.previousAdmin;
...
router.post('/', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const memberObjectIds = memberList.map((id) => new mongoose_1.Types.ObjectId(id));
        ...
        const foundUsers = await User_1.default.find({ _id: { $in: allMemberIds } });
        ...
        let joinCode;
        try {
            joinCode = await Group_1.default.generateInviteCode();
        ...
        const populatedGroup = await Group_1.default.findById(group._id)
            .populate('members', 'name phone profilePicture')
            .populate('admin', 'name phone');
            ...
            setImmediate(async () => {
                try {
                    ...
                    for (const memberId of memberList) {
                        try {
                            const member = await User_1.default.findById(new mongoose_1.Types.ObjectId(memberId));
                            if (member?.pushToken && member.pushToken !== 'expo-go-local-mode') {
                                await (0, pushNotifications_1.sendGroupInviteNotification)(member.pushToken, adminName, group.name, group._id.toString(), adminId);
...
router.post('/join', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const group = await Group_1.default.findOne({ joinCode: code.toUpperCase() });
        if (!group) {
            return res.status(404).json({ error: 'Invalid invite code' });
        }
        if (group.members.includes(new mongoose_1.Types.ObjectId(userId))) {
            return res.status(400).json({ error: 'You are already a member of this group' });
        }
        // Add user to group
        const updatedGroup = await Group_1.default.findByIdAndUpdate(group._id, {
            $push: { members: new mongoose_1.Types.ObjectId(userId) },
            updatedAt: new Date()
        }, { new: true }).populate('members', 'name phone profilePicture')
            .populate('admin', 'name phone');
        if (!updatedGroup) {
...
router.put('/:id/transfer-admin', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        group.adminTransferInfo = {
            previousAdmin: new mongoose_1.Types.ObjectId(userId),
            transferredAt: new Date(),
            seen: false
        };
        group.admin = new mongoose_1.Types.ObjectId(newAdminId);
        group.updatedAt = new Date();
        await group.save();
        // Get user details for notification
        const newAdmin = await User_1.default.findById(newAdminId).select('name');
        const currentAdmin = await User_1.default.findById(userId).select('name');
        ...
        res.json({
            ...
            group: await Group_1.default.findById(groupId)
                .populate('members', 'name phone profilePicture')
                .populate('admin', 'name phone')
        });
    }
...
router.put('/:id/mark-transfer-seen', auth_1.requireAuth, async (req, res) => {
    try {
        const groupId = req.params.id;
        const userId = req.userId;
        if (!userId) {
...
router.put('/:id/remove-member', auth_1.requireAuth, async (req, res) => {
    try {
        const groupId = req.params.id;
        const userId = req.userId;
        ...
        await Group_1.default.findByIdAndUpdate(groupId, {
            $pull: { members: new mongoose_1.Types.ObjectId(memberId) },
            updatedAt: new Date()
        });
        console.log(`✅ Member ${memberId} removed from group ${groupId}`);
        // Get updated group with populated members
        const updatedGroup = await Group_1.default.findById(groupId)
            .populate('members', 'name phone profilePicture')
            .populate('admin', 'name phone');
        res.json({
            success: true,
...
router.delete('/:id', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (group.admin.toString() === userId) {
            ...
            await Group_1.default.findByIdAndUpdate(groupId, {
                $pull: { members: new mongoose_1.Types.ObjectId(userId) },
                updatedAt: new Date()
            });
            return res.json({
                success: true,
...
router.delete('/', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const userGroups = await Group_1.default.find({ members: new mongoose_1.Types.ObjectId(userId) });
        for (const group of userGroups) {
            if (group.admin.toString() === userId) {
                ...
                await Group_1.default.findByIdAndUpdate(group._id, {
                    $pull: { members: new mongoose_1.Types.ObjectId(userId) },
                    updatedAt: new Date()
                });
            }
...
router.post('/:id/call/initiate', auth_1.requireAuth, async (req, res) => {
    try {
        const groupId = req.params.id;
        const userId = req.userId;
        ...
        // Get user details
        const caller = await User_1.default.findById(userId).select('name profilePicture');
        if (!caller) {
            return res.status(404).json({ error: 'User not found' });
        }
        // Generate call session data
        const callSession = {
            callId: new mongoose_1.Types.ObjectId().toString(),
            groupId: groupId,
            initiatorId: userId,
...
router.post('/:id/call/join', auth_1.requireAuth, async (req, res) => {
    try {
        const groupId = req.params.id;
        const userId = req.userId;
        ...
        // Get user details
        const user = await User_1.default.findById(userId).select('name profilePicture');
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
...
router.post('/:id/call/leave', auth_1.requireAuth, async (req, res) => {
    try {
        const groupId = req.params.id;
        const userId = req.userId;
        const { callId } = req.body;
...
router.post('/:id/call/end', auth_1.requireAuth, async (req, res) => {
    try {
        const groupId = req.params.id;
        const userId = req.userId;
        const { callId } = req.body;
...
router.get('/:id/call/status', auth_1.requireAuth, async (req, res) => {
    try {
        const groupId = req.params.id;
        const userId = req.userId;
        if (!userId) {
