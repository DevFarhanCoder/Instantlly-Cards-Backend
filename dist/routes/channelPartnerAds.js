...
const uploadVideos = (0, multer_1.default)({
    storage: multer_1.default.memoryStorage(),
    limits: {
...
const uploadMixed = (0, multer_1.default)({
    storage: multer_1.default.memoryStorage(),
    limits: {
 ...
 */
router.post('/', exports.upload.any(), // Max 5 images
async (req, res) => {
    try {
        ...
        let bottomImageS3Result = null;
        let fullscreenImageS3Result = null;
        let bottomVideoS3Result = null;
        let fullscreenVideoS3Result = null;
        // Validate dates
        ...
        if (!user) {
            ...
            user = await User_1.default.findOne({ phone: uploaderPhone });
            if (!user && uploaderPhone.startsWith('+91')) {
                user = await User_1.default.findOne({ phone: uploaderPhone.substring(3) });
            }
            if (!user && !uploaderPhone.startsWith('+')) {
                user = await User_1.default.findOne({ phone: '+91' + uploaderPhone });
            }
            if (!user && cleanPhone.length >= 10) {
                const last10Digits = cleanPhone.slice(-10);
                user = await User_1.default.findOne({ phone: { $regex: last10Digits + '$' } });
            }
        }
        ...
        console.log(`Deducted 1200 credits from ${user.name} (${uploaderPhone}). Balance: ${currentCredits} to ${user.credits}`);
        // ---- IMAGES â†’ S3 ----
        const tempAdId = Date.now().toString();
        if (bottomMediaType === "image" && bottomImage) {
            bottomImageS3Result = await (0, s3Service_1.uploadPendingAdMedia)(bottomImage.buffer, bottomImage.originalname, uploaderPhone, tempAdId, bottomImage.mimetype);
            console.log('âœ… Bottom image uploaded to S3:', bottomImageS3Result.url);
        }
        if (fullscreenMediaType === "image" && fullscreenImage) {
            fullscreenImageS3Result = await (0, s3Service_1.uploadPendingAdMedia)(fullscreenImage.buffer, fullscreenImage.originalname, uploaderPhone, tempAdId, fullscreenImage.mimetype);
 ...
 * NO AUTH REQUIRED - Anyone can create ads, admin will review
 */
router.post('/video', uploadVideos.array('videos', 2), // Max 2 videos (bottom + fullscreen)
async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            if (!user && uploaderPhone.startsWith('+91')) {
                ...
                user = await User_1.default.findOne({ phone: phoneWithoutPrefix });
            }
            // Try with +91 prefix
            if (!user && !uploaderPhone.startsWith('+')) {
                const phoneWithPrefix = '+91' + uploaderPhone;
                user = await User_1.default.findOne({ phone: phoneWithPrefix });
            }
            ...
            if (!user && cleanPhone.length >= 8) {
                ...
                user = await User_1.default.findOne({ phone: { $regex: cleanPhone } });
            }
        }
        console.log('?? Found user:', user ? `${user.name} (${user.phone}) with ${user.credits} credits` : 'NOT FOUND');
        if (!user) {
        ...
        console.log(`? Deducted 1200 credits from ${user.name} (${uploaderPhone}). Remaining: ${user.credits}`);
        ...
        const bucket = new mongodb_1.GridFSBucket(db, { bucketName: 'adImages' });
        ...
        const bottomReadable = stream_1.Readable.from(bottomVideoFile.buffer);
        const bottomVideoId = await new Promise((resolve, reject) => {
            bottomReadable
                .pipe(bottomVideoStream)
                .on('finish', () => resolve(bottomVideoStream.id))
                .on('error', reject);
        });
        ...
        if (files.length > 1) {
            ...
            const fullscreenReadable = stream_1.Readable.from(fullscreenFile.buffer);
            fullscreenVideoId = await new Promise((resolve, reject) => {
                fullscreenReadable
                    .pipe(fullscreenStream)
                    .on('finish', () => resolve(fullscreenStream.id))
                    .on('error', reject);
            });
...
router.get('/video/:id', async (req, res) => {
    try {
        ...
        if (range) {
            ...
            bucket.openDownloadStream(new mongodb_1.ObjectId(id), { start, end: end + 1 }).pipe(res);
        }
        else {
 ...
 */
router.put('/design-requests/:id/assign', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const designRequest = await DesignRequest_1.default.findById(id);
        if (!designRequest) {
...
router.get('/image/:id', async (req, res) => {
    try {
        ...
        for (const bucketName of bucketNames) {
            try {
                const bucket = bucketName === 'fs'
                    ? new mongodb_1.GridFSBucket(db)
                    : new mongodb_1.GridFSBucket(db, { bucketName });
                const files = await bucket.find({ _id: new mongodb_1.ObjectId(id) }).toArray();
                if (files.length > 0) {
                    foundBucket = bucket;
                    break;
        ...
        const downloadStream = foundBucket.openDownloadStream(new mongodb_1.ObjectId(id));
...
router.get('/', async (req, res) => {
    try {
        ...
        const adsWithDetails = ads.map((ad) => {
            ...
            const bottomS3Url = ad.bottomImageS3?.url || null;
            const fullscreenS3Url = ad.fullscreenImageS3?.url || null;
            const bottomVidS3Url = ad.bottomVideoS3?.url || null;
            const fullscreenVidS3Url = ad.fullscreenVideoS3?.url || null;
            return {
                id: ad._id,
                title: ad.title,
                ...
                hasBottomVideo: !!(ad.bottomVideoGridFS || bottomVidS3Url),
                hasFullscreenVideo: !!(ad.fullscreenVideoGridFS || fullscreenVidS3Url),
 ...
 */
router.put('/:id', exports.upload.array('images', 5), async (req, res) => {
    try {
        const { id } = req.params;
        if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
        ...
        if (files && files.length > 0) {
            ...
            if (ad.bottomImageGridFS)
                oldImageIds.push(ad.bottomImageGridFS);
            if (ad.fullscreenImageGridFS)
                oldImageIds.push(ad.fullscreenImageGridFS);
            ...
            const bottomReadable = stream_1.Readable.from(bottomImageFile.buffer);
            const bottomImageId = await new Promise((resolve, reject) => {
                bottomReadable
                    .pipe(bottomImageStream)
                    .on('finish', () => resolve(bottomImageStream.id))
                    .on('error', reject);
            });
            ...
            if (files.length > 1) {
                ...
                const fullscreenReadable = stream_1.Readable.from(fullscreenFile.buffer);
                const fullscreenImageId = await new Promise((resolve, reject) => {
                    fullscreenReadable
                        .pipe(fullscreenStream)
                        .on('finish', () => resolve(fullscreenStream.id))
                        .on('error', reject);
 ...
 */
router.post('/design-request', uploadMixed.fields([
    ...
    try {
        ...
        const referenceImagesS3 = [];
        const referenceVideosS3 = [];
        // Generate a temporary request ID for S3 key organization
        const tempRequestId = new mongodb_1.ObjectId().toString();
        ...
        if (hasImages) {
            for (let i = 0; i < files.referenceImages.length; i++) {
                ...
                const s3Result = await (0, s3Service_1.uploadDesignRequestMedia)(imageFile.buffer, filename, userId || 'unknown', uploaderPhone, tempRequestId, imageFile.mimetype);
        ...
        if (hasVideos) {
            for (let i = 0; i < files.referenceVideos.length; i++) {
                ...
                const s3Result = await (0, s3Service_1.uploadDesignRequestMedia)(videoFile.buffer, filename, userId || 'unknown', uploaderPhone, tempRequestId, videoFile.mimetype);
                referenceVideosS3.push(s3Result);
                console.log(`? Uploaded reference video ${i + 1} to S3:`, s3Result.key);
            }
        }
        // Create design request
        const designRequest = new DesignRequest_1.default({
 ...
 */
router.get('/design-requests/all', async (req, res) => {
    try {
        console.log('Fetching all design requests...');
        const designRequests = await DesignRequest_1.default.find({}).sort({ createdAt: -1 }).limit(500);
...
router.get('/design-requests', async (req, res) => {
    try {
        ...
        const designRequests = await DesignRequest_1.default.find({ uploaderPhone: phone })
            .sort({ createdAt: -1 })
            .limit(50);
        res.json({
            success: true,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (db) {
            ...
            if (ad.bottomImageGridFS) {
                imageIdsToDelete.push(ad.bottomImageGridFS);
            }
            if (ad.fullscreenImageGridFS) {
                imageIdsToDelete.push(ad.fullscreenImageGridFS);
            }
...
// ==========================================
router.get('/designs-for-approval', async (req, res) => {
    try {
            ...
            return {
                ...
                status: upload.status === 'sent-to-user' ? 'pending-approval'
                    : upload.status === 'approved' ? 'approved'
                        : 'changes-requested',
                designFiles: (upload.filesS3 || []).map((f) => ({
                    url: f.url,
...
router.post('/designs/:id/approve', async (req, res) => {
    try {
        ...
        const upload = await DesignerUpload_1.default.findById(id);
        if (!upload) {
            return res.status(404).json({ message: 'Design not found' });
        }
        upload.status = 'approved';
        await upload.save();
        const designRequest = await DesignRequest_1.default.findById(upload.designRequestId);
        if (designRequest) {
...
// ==========================================
router.post('/designs/:id/request-changes', async (req, res) => {
    try {
        const { id } = req.params;
