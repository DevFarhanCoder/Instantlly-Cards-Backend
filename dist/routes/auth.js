...
router.post("/signup", async (req, res) => {
    ...
    try {
        ...
        const existingUser = await User_1.default.findOne({ phone: cleanPhone });
        if (existingUser) {
        ...
        if (cleanReferralCode) {
            referrer = await User_1.default.findOne({ referralCode: cleanReferralCode });
            if (!referrer) {
        ...
        const hashedPassword = await bcryptjs_1.default.hash(cleanPassword, 12);
        ...
        if (referrer) {
            ...
            userData.level = Math.min(referrer.level || 0, 9) + 1;
        }
        ...
        if (savedUser.parentId) {
            try {
                const { updateAncestorDownlineCounts } = await import("../services/mlm/downlineService.js");
        ...
        try {
            ...
            if (cleanPhone.startsWith("+")) {
                ...
                if (phoneWithoutPlus.startsWith("91") &&
            ...
            const upsertResult = await Card_1.default.findOneAndUpdate(cardFilter, cardUpdate, upsertOptions);
            if (!upsertResult) {
            ...
            const cleanupResult = await Card_1.default.deleteMany({
                userId: userIdStr,
            ...
            const finalCount = await Card_1.default.countDocuments({ userId: userIdStr });
            console.log(`🔖 [REQ:${reqId}] 🔍 FINAL VERIFICATION: ${finalCount} card(s) total`);
            const defaultCount = await Card_1.default.countDocuments({
                userId: userIdStr,
                isDefault: true,
            });
            ...
            try {
                const existingCard = await Card_1.default.findOne({
                    userId: savedUser._id.toString(),
                })
                    .sort({ createdAt: 1 })
                    .lean();
        ...
        try {
            const contactsWithThisNumber = await Contact_1.default.find({
            ...
            if (contactsWithThisNumber.length > 0) {
                ...
                await Contact_1.default.updateMany({ phoneNumber: cleanPhone }, {
                    $set: {
                        isAppUser: true,
                ...
                for (const contact of contactsWithThisNumber) {
                    const contactOwner = contact.userId;
                    if (contactOwner &&
                        contactOwner.pushToken &&
                        contactOwner.pushToken !== "expo-go-local-mode") {
                        try {
                            await (0, pushNotifications_1.sendContactJoinedNotification)(contactOwner.pushToken, cleanName, cleanPhone, savedUser._id.toString());
        ...
        const userResponse = {
            ...
            about: savedUser.about || "Available",
...
router.post("/login", async (req, res) => {
    try {
        ...
        const user = await User_1.default.findOne({ phone: { $in: phoneVariants } }).select("+password");
        if (!user) {
        ...
        const token = jsonwebtoken_1.default.sign({ sub: user._id.toString(), phone: user.phone }, process.env.JWT_SECRET, { expiresIn: "365d" });
...
router.get("/profile", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const profileData = {
            ...
            about: user.about || "Available",
            credits: user.credits || 0,
            creditsExpiryDate: user.creditsExpiryDate,
            referralCode: user.referralCode,
            gender: user.gender,
        ...
        console.log("Credits expiry:", user.creditsExpiryDate);
...
router.put("/update-profile", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (phone !== undefined) {
            ...
            const existingUser = await User_1.default.findOne({ phone: normalizedPhone });
        ...
        if (profilePicture !== undefined) {
            try {
                ...
                await Contact_1.default.updateMany({ appUserId: userId }, { $set: { profilePicture: updateData.profilePicture } });
...
router.post("/update-service-type", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const user = await User_1.default.findByIdAndUpdate(userId, { serviceType }, { new: true });
        if (!user) {
            return res.status(404).json({ message: "User not found" });
        }
...
router.post("/upload-profile-picture", auth_1.requireAuth, upload.single("profilePicture"), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const user = await User_1.default.findByIdAndUpdate(userId, { profilePicture: profilePictureUrl }, { new: true });
        if (!user) {
            return res.status(404).json({ message: "User not found" });
        }
        res.json({
...
router.post("/check-phone", async (req, res) => {
    try {
        ...
        console.log(`[CHECK-PHONE] 📱 New signup - sending OTP to ${normalizedPhone}`);
        const otp = Math.floor(100000 + Math.random() * 900000).toString();
        otpService_1.otpService.storeOTP(normalizedPhone, otp);
        const fast2smsApiKey = process.env.FAST2SMS_API_KEY;
        if (!fast2smsApiKey) {
...
${finalAppHash}`;
            ...
            const fast2smsResponse = await axios_1.default.get(`https://www.fast2sms.com/dev/bulkV2?${fast2smsPayload.toString()}`, {
...
router.post("/send-reset-otp", async (req, res) => {
    try {
        ...
        const user = await User_1.default.findOne({ phone: { $in: phoneVariants } });
        if (!user) {
        ...
        try {
            ...
            const fast2smsResponse = await axios_1.default.get(`https://www.fast2sms.com/dev/bulkV2?${fast2smsPayload.toString()}`, { headers: { "Cache-Control": "no-cache" }, timeout: 10000 });
...
router.post("/verify-otp", async (req, res) => {
    try {
        ...
        const isValid = otpService_1.otpService.verifyOTP(normalizedPhone, otp);
        if (!isValid) {
        ...
        const resetToken = jsonwebtoken_1.default.sign({ phone: normalizedPhone, purpose: "reset" }, process.env.JWT_SECRET, { expiresIn: "15m" });
        return res.json({
            success: true,
...
router.get("/users/:userIdOrPhone", async (req, res) => {
    try {
        ...
        if (isValidObjectId) {
            ...
            user = await User_1.default.findById(userIdOrPhone).select("_id name phone profilePicture about");
...
router.get("/version-check", async (req, res) => {
    try {
        ...
        const LATEST_VERSIONS = {
            android: "1.0.73", // ← Update this when publishing new version to Play Store
            ios: "1.0.73", // ← Update this when publishing new version to App Store
        };
        ...
        const latestVersion = LATEST_VERSIONS[requestedPlatform];
        if (!latestVersion) {
