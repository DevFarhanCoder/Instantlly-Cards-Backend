...
const createContactJoinedNotification = async (userId, contactName, contactUserId) => {
    try {
...
router.post("/sync", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const appUsers = await User_1.default.find({
...
router.get("/app-users", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const contacts = appUsers.map(user => ({
...
router.post("/sync-all", auth_1.requireAuth, async (req, res) => {
    try {
        const { contacts } = req.body;
        const userId = req.userId;
        ...
        const shaliniUser = appUsers.find((u) => u.name && u.name.toLowerCase().includes('shalini'));
        if (shaliniUser) {
        ...
        if (contactsToSave.length > 0) {
            const writeResult = await Contact_1.default.bulkWrite(contactsToSave, { ordered: false });
        ...
        const shaliniContact = await Contact_1.default.findOne({
            userId,
...
router.post("/sync-incremental", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        const startTime = Date.now();
        console.log(`🔄 [${userId}] Incremental contact sync started`);
        // Get all existing contacts for this user
        const existingContacts = await Contact_1.default.find({ userId })
        ...
        if (updateOperations.length > 0) {
            await Contact_1.default.bulkWrite(updateOperations, { ordered: false });
...
router.get("/all", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const limit = parseInt(req.query.limit) || 1000; // Default 1000 contacts per page
        ...
        const [totalContacts, contacts] = await Promise.all([
            ...
            Contact_1.default.find(query)
                .populate({ path: 'appUserId', select: 'name profilePicture about', model: User_1.default })
        ...
        const appUsersCount = formattedContacts.filter((c) => c.isAppUser).length;
...
router.post("/refresh-app-status", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
        ...
        if (bulkOps.length > 0) {
            await Contact_1.default.bulkWrite(bulkOps);
        }
...
router.get("/categorized", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
        ...
        const contacts = await Contact_1.default.find({ userId })
            .populate({ path: 'appUserId', select: 'name phone profilePicture about', model: User_1.default })
            .sort({ name: 1 })
            .lean();
        ...
        contacts.forEach(contact => {
            ...
            if (contact.isAppUser && contact.appUserId) {
                const appUserData = contact.appUserId;
                appUsers.push({
                    ...contactData,
...
router.post("/smart-sync", auth_1.requireAuth, async (req, res) => {
    try {
        const { contacts } = req.body;
        const userId = req.userId;
        if (!contacts || !Array.isArray(contacts)) {
        ...
        const existingContacts = await Contact_1.default.find({ userId }).select('phoneNumber').lean();
        ...
        const newContacts = normalizedContacts.filter(contact => !existingPhoneNumbers.has(contact.phoneNumber));
        ...
        await Contact_1.default.insertMany(contactsToSave);
        // Count app users in new contacts
        const newAppUserContacts = newContacts.filter(contact => phoneToUserMap.has(contact.phoneNumber));
        res.json({
            success: true,
