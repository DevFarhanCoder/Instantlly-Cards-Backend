...
router.get("/stats", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const totalUnread = await Notification_1.default.countDocuments({ userId, read: false });
        const totalCount = await Notification_1.default.countDocuments({ userId });
        res.json({
            success: true,
            data: {
                total: totalCount,
...
router.post("/register-token", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ error: "User not authenticated" });
