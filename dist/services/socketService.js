...
class SocketService {
    ...
    setupSocketAuthentication() {
        this.io.use(async (socket, next) => {
            try {
                ...
                const user = await User_1.default.findById(userId).select('name email profilePicture');
                if (!user) {
    ...
    setupSocketEvents() {
        this.io.on('connection', (socket) => {
            ...
            // ✅ CLIENT DELIVERY ACK
            socket.on('message_delivered_ack', async ({ messageId }) => {
                try {
    ...
    async handleUserJoin(socket) {
        ...
        try {
            const pendingMessages = await Message_1.default.find({
    ...
    async handlePrivateMessage(socket, data) {
        try {
            ...
            const chat = await Chat_1.default.findOrCreateConversation(senderId, data.receiverId);
            // Create message
            ...
            await message.save();
            await Chat_1.default.updateLastMessage(chat._id?.toString() || '', message._id?.toString() || '');
    ...
    async handleGroupMessage(socket, data) {
        try {
            ...
            const group = await Group_1.default.findById(data.groupId);
            if (!group) {
                socket.emit('error', { message: 'Group not found' });
                return;
            }
            const senderId = socket.userId;
            const isMember = Array.isArray(group.members) && group.members.some((m) => m.toString() === senderId);
            if (!isMember) {
            ...
            const offlineMembers = group.members.filter(memberId => memberId.toString() !== senderId && !this.onlineUsers.has(memberId.toString()));
    ...
    async handleMessageRead(socket, data) {
        try {
            ...
            if (data.conversationId) {
                await Chat_1.default.markAsRead(data.conversationId, socket.userId);
            }
        }
        catch (error) {
    ...
    async handleInitiateGroupCall(socket, data) {
        try {
            console.log(`📞 ${socket.user.name} initiating ${data.callType} call in group ${data.groupId}`);
            // Verify user is member of group
            const group = await Group_1.default.findById(data.groupId);
            if (!group || !group.members.includes(new mongoose_1.default.Types.ObjectId(socket.userId))) {
                socket.emit('error', { message: 'You are not a member of this group' });
                return;
            }
            // Generate call session
            const callId = new mongoose_1.default.Types.ObjectId().toString();
            const callSession = {
                callId: callId,
                groupId: data.groupId,
    ...
    async handleEndGroupCall(socket, data) {
        try {
    ...
    handleGroupCallSignal(socket, data) {
        try {
