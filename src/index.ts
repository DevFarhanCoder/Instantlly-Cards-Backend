...
app.use((req, res, next) => {
  ...
  console.log(`🔑 Auth: ${req.headers.authorization ? 'Bearer ***' + req.headers.authorization.slice(-8) : 'None'}`);
  
  // Log response when it finishes
  const originalSend = res.send;
  res.send = function(data) {
    const responseTime = Date.now() - Date.now(); // This won't work properly but shows concept
    console.log(`📤 [RESPONSE] ${method} ${path} -> ${res.statusCode} (${typeof data === 'string' ? data.length : 0} bytes)`);
