...
r.get("/feed/public", async (_req, res) => {
  ...
  console.log(`\n🌍 [PUBLIC FEED] ${timestamp}`);
  console.log(`📱 Request from IP: ${_req.ip || 'unknown'}`);
  console.log(`🌐 User Agent: ${_req.headers['user-agent'] || 'unknown'}`);
  
  const items = await Card.find({}).sort({ createdAt: -1 }).limit(50).lean();
  console.log(`📄 Found ${items.length} cards in public feed`);
  console.log(`🌍 [PUBLIC FEED END] Returning ${items.length} cards\n`);
...
r.get("/feed/contacts", async (req: AuthReq, res) => {
  try {
    ...
    // Get user info first
    const user: any = await User.findById(userId).select('name phone').lean();
    console.log(`👤 User Info: ${user?.name} (${user?.phone})`);
    
    // Get all contacts who are app users (optimized query with select)
    console.log(`🔍 Step 1: Finding user's contacts who are app users...`);
    const myContacts = await Contact.find({ 
      userId,
      isAppUser: true 
    })
    .select('appUserId name phoneNumber')
    .lean()
    .exec();
    
    console.log(`📞 Found ${myContacts.length} contacts who are app users:`);
    myContacts.forEach((contact: any, index) => {
      console.log(`   ${index + 1}. ${contact.name} (${contact.phoneNumber}) -> App User ID: ${contact.appUserId}`);
    });
    ...
    console.log(`🎯 Step 2: Looking for cards from these user IDs:`, allUserIds);
    ...
    console.log(`📇 Step 3: Found ${allCards.length} total cards:`);
    ...
    console.log(`📊 Cards breakdown:`);
    console.log(`   🏠 Own cards: ${ownCards.length}`);
    console.log(`   👥 Contact cards: ${contactCards.length}`);
    console.log(`   📋 Total: ${allCards.length}`);
    
    // Log sample cards for debugging
    console.log(`📄 Sample cards being returned:`);
    allCards.slice(0, 3).forEach((card: any, index) => {
      console.log(`   ${index + 1}. ${card.name} - ${card.companyName || 'No Company'} (Owner: ${card.userId})`);
    });
    
    const elapsed = Date.now() - startTime;
    console.log(`⏱️ Feed loaded in ${elapsed}ms`);
    console.log(`🔥 [CONTACT FEED DEBUG END] Returning ${allCards.length} cards\n`);
    ...
    console.error("❌ Error stack:", err instanceof Error ? err.stack : 'Unknown stack');
    res.status(500).json({ 
      success: false,
