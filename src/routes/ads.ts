...
router.get("/active", async (req: Request, res: Response) => {
  try {
    ...
    const ads = await Ad.find({
      ...
      .select(
        "title phoneNumber priority impressions clicks startDate endDate bottomImageGridFS fullscreenImageGridFS bottomMediaType bottomVideoUrl fullscreenMediaType fullscreenVideoUrl",
      ) // Exclude large base64 fields
    ...
    if (ads.length > 0) {
      console.log("📋 First ad sample:", {
        ...
        fullscreenImageGridFS:
          ads[0].fullscreenImageGridFS?.toString() || "NULL",
      });
    }
...
router.get("/image/:id/:type", async (req: Request, res: Response) => {
  ...
  try {
    ...
    while (retries < maxRetries && !ad) {
      try {
        ad = await Ad.findById(id)
          .select(
            "bottomImageGridFS fullscreenImageGridFS videoThumbnailGridFS bottomImage fullscreenImage",
          )
        ...
        if (!ad && retries < maxRetries - 1) {
          retries++;
          console.warn(
            `⚠️  [IMG RETRY] Database query failed, retry ${retries}/${maxRetries}`,
          );
        ...
        retries++;
        console.error(
          `❌ [IMG DB ERROR] Attempt ${retries}/${maxRetries}:`,
          dbError.message,
        );
    ...
    const gridfsId =
      type === "bottom" ? ad.bottomImageGridFS : ad.fullscreenImageGridFS;

    if (!gridfsId) {
      console.warn(
        "⚠️  [IMG STEP 4] No GridFS ID found - Checking for legacy base64",
      );

      // Fallback to base64 if GridFS migration not complete
      const base64Data =
        type === "bottom" ? ad.bottomImage : ad.fullscreenImage;

      if (!base64Data || base64Data.length === 0) {
        console.error(
          "❌ [IMG ERROR] No image found (neither GridFS nor base64)",
        );
        console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
        return res.status(404).json({
          success: false,
    ...
    try {
      ...
      if (!fileExists) {
        console.error(
          "❌ [IMG ERROR] GridFS file not found:",
          gridfsId.toString(),
        );
        console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
        return res.status(404).json({
      ...
      console.error(
        "❌ [IMG ERROR] Failed to check GridFS file existence:",
        checkError.message,
      );
    ...
    try {
      ...
      console.error(
        "❌ [IMG ERROR] File not found in GridFS:",
        gridfsId.toString(),
      );
      console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
      return res.status(404).json({
    ...
    try {
      ...
      console.error(
        "❌ [IMG ERROR] Failed to create download stream:",
        streamError.message,
      );
      console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
...
router.post("/track-impression/:id", async (req: Request, res: Response) => {
  try {
    ...
    await Ad.findByIdAndUpdate(id, { $inc: { impressions: 1 } }, { new: true });
...
router.post("/track-click/:id", async (req: Request, res: Response) => {
  try {
    ...
    await Ad.findByIdAndUpdate(id, { $inc: { clicks: 1 } }, { new: true });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const fullscreenVideo = files?.find(
        (f) => f.fieldname === "fullscreenVideo",
      );
      const bottomImageFile = files?.find((f) => f.fieldname === "bottomImage");
      const fullscreenImageFile = files?.find(
        (f) => f.fieldname === "fullscreenImage",
      );
      ...
      if (
        ...
        console.error(
          "❌ Invalid fullscreen video format:",
          fullscreenVideo.mimetype,
        );
        return res
          .status(400)
...
router.get("/my-ads", async (req: Request, res: Response) => {
  try {
    ...
    if (!phoneNumber) {
      ...
      console.log(
        "🔎 [my-ads-public] Authorization header present:",
        !!authHeader,
      );
      if (auth) {
        try {
          ...
          if (payload?.phone) phoneNumber = payload.phone;
            ...
            try {
              const user = (await User.findById(payload.sub)
                ...
                .lean()) as any;
              console.log(
                "🔍 [my-ads-public] Loaded user for phone extraction:",
                { found: !!user, phone: user?.phone },
              );
              if (user?.phone) phoneNumber = user.phone;
            } catch (uErr) {
              console.warn(
                "Could not load user for phone extraction from token:",
          ...
          console.warn(
            "⚠️ [my-ads-public] jwt.verify failed:",
            (tErr as any)?.message || tErr,
          );
          try {
            const decoded: any = jwt.decode(auth);
            console.log(
              "🔎 [my-ads-public] jwt.decode fallback payload preview:",
            ...
            if (decoded?.phone) phoneNumber = decoded.phone;
              ...
              try {
                const user = (await User.findById(decoded.sub)
                  ...
                  .lean()) as any;
                console.log(
                  "🔍 [my-ads-public] Loaded user (decode fallback):",
                  { found: !!user, phone: user?.phone },
                );
                if (user?.phone) phoneNumber = user.phone;
              } catch (uErr) {
                console.warn(
                  "Could not load user for phone extraction from decoded token:",
                  (uErr as any)?.message || uErr,
                );
              }
            }
            ...
            console.warn(
              "Could not decode token for phone extraction:",
              (decodeErr as any)?.message || decodeErr,
            );
          }
        }
      }
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `✅ Found ${ads.length} ads (page ${page} of ${Math.ceil(totalAds / limit)})`,
    );
...
router.get("/analytics/summary", async (req: AdminAuthReq, res: Response) => {
  try {
    ...
    const summary = {
      ...
      clickThroughRate:
        analytics[0]?.totalImpressions > 0
          ? (
  ...
  try {
    ...
    if (ad.bottomImageGridFS) {
      try {
        ...
        console.log(
          `🗑️ Deleted bottom image from GridFS: ${ad.bottomImageGridFS}`,
        );
      } catch (error) {
    ...
    if (ad.fullscreenImageGridFS) {
      try {
        ...
        console.log(
          `🗑️ Deleted fullscreen image from GridFS: ${ad.fullscreenImageGridFS}`,
        );
      } catch (error) {
