...
async function resolveVoucherScopeForAdmin(
  ...
  if (!voucherId || !mongoose.isValidObjectId(voucherId)) return null;

  const voucherDoc = await Voucher.findById(voucherId)
    .select("_id templateId")
    .lean();

  if (!voucherDoc) {
    return new mongoose.Types.ObjectId(voucherId);
  }

  return (
    ((voucherDoc as any).templateId as mongoose.Types.ObjectId) ||
    ((voucherDoc as any)._id as mongoose.Types.ObjectId)
  );
}
...
const adminAuth = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (adminKey && validKeys.includes(adminKey as string)) {
    ...
    console.log(
      `âŒ Admin auth failed. Received key: ${adminKey}, Expected one of: ${validKeys.join(", ")}`,
    );
    res
      .status(401)
...
router.get("/users", adminAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `âœ… Found ${users.length} users (Total: ${total}). First user credits: ${users[0]?.credits}, Last user credits: ${users[users.length - 1]?.credits}`,
    );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=phone-numbers-${Date.now()}.csv`,
      );
      res.send(csvContent);
    } catch (error) {
...
router.get(
  "/ads/pending",
  requireAdminAuth,
  async (req: AdminAuthReq, res: Response) => {
    try {
      ...
      const adsWithDetails = pendingAds.map((ad) => {
        ...
        if (!detectedAdType) {
          ...
          if (
            (ad as any).bottomVideoGridFS ||
            (ad as any).fullscreenVideoGridFS
          ) {
...
router.post(
  ...
  async (req: AdminAuthReq, res: Response) => {
    try {
      ...
      console.log(
        "Credits already deducted at upload. Proceeding with approval only.",
      );
      ...
      if ((ad as any).bottomImageS3?.key) {
        const filename =
          (ad as any).bottomImageS3.key.split("/").pop() || "bottom_image.jpg";
        mediaFiles.push({ filename, type: "bottom_image" });
      }

      if ((ad as any).fullscreenImageS3?.key) {
        const filename =
          (ad as any).fullscreenImageS3.key.split("/").pop() ||
      ...
      if ((ad as any).bottomVideoS3?.key) {
        const filename =
          (ad as any).bottomVideoS3.key.split("/").pop() || "bottom_video.mp4";
        mediaFiles.push({ filename, type: "bottom_video" });
      }

      if ((ad as any).fullscreenVideoS3?.key) {
        const filename =
          (ad as any).fullscreenVideoS3.key.split("/").pop() ||
      ...
      if (mediaFiles.length > 0) {
        console.log(
          `ðŸ“¦ Moving ${mediaFiles.length} media files from pending to approved in S3...`,
        );

        try {
          const approvedFiles = await movePendingToApproved(
            id,
            uploaderPhone,
            mediaFiles,
          );
          ...
          console.log(
            `âœ… Successfully moved all media to approved-ads/${id}/`,
          );
        } catch (error) {
        ...
        if (db) {
          ...
          if (ad.bottomImageGridFS) {
            try {
              ...
              console.log(
                `ðŸ—‘ï¸ Deleted bottom image: ${ad.bottomImageGridFS}`,
              );
            } catch (err) {
          ...
          if (ad.fullscreenImageGridFS) {
            try {
              ...
              console.log(
                `ðŸ—‘ï¸ Deleted fullscreen image: ${ad.fullscreenImageGridFS}`,
              );
            } catch (err) {
          ...
          if ((ad as any).bottomVideoGridFS) {
            try {
              ...
              console.log(
                `ðŸ—‘ï¸ Deleted bottom video: ${(ad as any).bottomVideoGridFS}`,
              );
            } catch (err) {
          ...
          if ((ad as any).fullscreenVideoGridFS) {
            try {
              ...
              console.log(
                `ðŸ—‘ï¸ Deleted fullscreen video: ${(ad as any).fullscreenVideoGridFS}`,
              );
            } catch (err) {
...
router.get(
  ...
  async (req: AdminAuthReq, res: Response) => {
    try {
      ...
      console.log(
        `ðŸ“‹ Admin ${req.adminUsername} fetching ads with filter:`,
        filter,
      );

      const ads = await Ad.find(filter).sort({ createdAt: -1 }).select("-__v");
...
router.get(
  ...
  async (req: AdminAuthReq, res: Response) => {
    try {
      ...
      const totalCredits =
        creditStats.length > 0 ? creditStats[0].totalCredits : 0;
  ...
  async (req: AdminAuthReq, res: Response) => {
    try {
      ...
      const user = await User.findById(userId).select(
        "name phone email credits profilePicture createdAt",
      );
...
router.post(
  ...
  async (req: AdminAuthReq, res: Response) => {
    try {
      ...
      console.log(
        `âœ… Admin transferred ${amount} credits to ${recipient.name}`,
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        console.log(
          "âš ï¸ Socket.IO not available for credit update notification",
        );
      }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const slotQuery: any = { ownerId: userId };
      if (voucherId) {
        try {
      ...
      const slots = await SpecialCredit.find(slotQuery).lean();

      const availableSlots = slots.filter((s: any) => s.status === "available");
      const sentSlots = slots.filter((s: any) => s.status === "sent");

      const availableCredits = availableSlots.reduce(
        (sum: number, s: any) => sum + (s.creditAmount || 0),
        0,
      );
      const distributedCredits = sentSlots.reduce(
        (sum: number, s: any) => sum + (s.creditAmount || 0),
        0,
      );

      const uniqueRecipients = new Set(
        sentSlots.map((s: any) => s.recipientId?.toString()).filter(Boolean),
      ).size;
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `ðŸ”„ Transferring application ${id} to position:`,
        newPositionId,
      );
    ...
    try {
      ...
      const topReferrersData = await Promise.all(
        usersWithReferrals.map(async (user: any) => {
          ...
          const creditsEarned = referralTransactions.reduce(
      ...
      const totalReferralCreditsGiven = totalReferralTransactions.reduce(
        (sum, tx: any) => sum + (tx.amount || 0),
        0,
      );

      const uniqueReferrers = topReferrers.length;
      const averageReferralsPerUser =
        uniqueReferrers > 0 ? totalReferrals / uniqueReferrers : 0;
    ...
    try {
      ...
      const referredUsersWithStats = await Promise.all(
        referredUsers.map(async (refUser: any) => {
          const refUserReferralCount = await User.countDocuments({
  ...
  try {
    ...
    const designerStats = await Promise.all(
      designers.map(async (d) => {
        const assignedCount = await DesignRequest.countDocuments({
          assignedDesignerId: d._id,
        });
        const completedCount = await DesignRequest.countDocuments({
          assignedDesignerId: d._id,
          status: "completed",
        });
        return {
...
router.get("/received-designs", async (req: Request, res: Response) => {
  try {
    const uploads = await DesignerUpload.find({})
      .populate(
        "designRequestId",
        "businessName adType uploaderName uploaderPhone status email phoneNumber",
      )
      .sort({ createdAt: -1 });
...
// ==========================================
router.post(
  "/received-designs/:id/send-to-user",
  async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      ...
      const designRequest = await DesignRequest.findById(
        designerUpload.designRequestId,
      );
      if (designRequest) {
 ...
 */
router.post("/vouchers", adminAuth, async (req: Request, res: Response) => {
  try {
    const {
      companyLogo,
      companyName,
    ...
    const voucherNumber = uuidv4().replace(/-/g, "").slice(0, 12).toUpperCase();
 ...
 */
router.get("/vouchers", adminAuth, async (req: Request, res: Response) => {
  try {
 ...
 */
router.put("/vouchers/:id", adminAuth, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const voucherTemplate = await Voucher.findById(id);
  ...
  try {
    ...
    let targetUser: any = userId
      ? await User.findById(userId as string)
          ...
          .lean()
      : await User.findOne({ isVoucherAdmin: true })
          .select("name phone level isVoucherAdmin specialCredits")
          .lean();

    console.log("👤 ADMIN MLM SLOTS - Target user", {
      userFound: !!targetUser,
      userId: (targetUser as any)?._id?.toString(),
    ...
    if (voucherId) {
      ...
      let rawVoucherId: mongoose.Types.ObjectId | null = null;
      if (mongoose.isValidObjectId(voucherId as string)) {
        rawVoucherId = new mongoose.Types.ObjectId(voucherId as string);
      }
      if (rawVoucherId) {
        query.voucherId = rawVoucherId;
      } else {
        const canonicalVoucherId = await resolveVoucherScopeForAdmin(
          voucherId as string,
        );
    ...
    const slots = await SpecialCredit.find(query)
    ...
    console.log("📦 ADMIN MLM SLOTS - Slots retrieved", {
      count: slots.length,
      slotNumbers: (slots as any[]).map((s) => s.slotNumber),
    });

    const creditPerSlot =
      SPECIAL_CREDIT_CHAIN_ADMIN[(targetUser as any).level || 0] ||
    ...
    console.log("✅ ADMIN MLM SLOTS - Success", {
      ...
      totalSlots: formattedSlots.length,
      availableSlots: formattedSlots.filter((s: any) => s.isAvailable).length,
    });
  } catch (error) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      let canonicalVoucherId: mongoose.Types.ObjectId | null = null;
      if (!mongoose.isValidObjectId(voucherId)) {
        ...
        canonicalVoucherId = new mongoose.Types.ObjectId(voucherId);
      }
      if (!canonicalVoucherId) {
        return res
          .status(404)
      ...
      const existingCount = await SpecialCredit.countDocuments({
        ownerId: adminUserId,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        success: true,
        message: `${count} slot(s) added for ${(voucher as any).companyName}`,
        slotsAdded: count,
