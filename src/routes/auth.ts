...
router.post("/signup", async (req, res) => {
  try {
    ...
    // If user has referral code, give them referralReward (300), otherwise signupBonus (200)
    const signupCredits = referrer
      ? creditConfig.referralReward
    ...
    try {
      ...
      defaultCard =
        createdCard && typeof createdCard.toObject === "function"
          ? createdCard.toObject()
          : createdCard;
      console.log(
        `🔖 [REQ:${reqId}] ✅ Default card ensured for user:`,
        savedUser._id,
        "phone:",
        phoneDigits,
      );
      ...
      if (cardError.code === 11000) {
        console.log(
          `🔖 [REQ:${reqId}] ℹ️ Default card already exists (caught duplicate key error)`,
        );
        ...
        try {
          ...
          console.error(
            `🔖 [REQ:${reqId}] ❌ Failed to fetch existing default card:`,
            fetchError,
          );
        }
      } else {
        console.error(
          `🔖 [REQ:${reqId}] ❌ Failed to ensure default card for new user:`,
          cardError,
        );
        ...
        try {
          ...
          console.log(
            `🔖 [REQ:${reqId}] ⚠️ Using fallback defaultCard for response (client will see a provisional card)`,
          );
        } catch (fallbackError) {
          console.error(
            `🔖 [REQ:${reqId}] ❌ Failed to create fallback defaultCard:`,
            fallbackError,
          );
          defaultCard = null;
        }
    ...
    const transactionDescription = referrer
      ? `Referral signup bonus - ${signupCredits} credits (joined using ${referrer.name}'s code)`
      : `Signup bonus - ${signupCredits} credits`;

    await Transaction.create({
      type: referrer ? "referral_signup_bonus" : "signup_bonus",
...
router.post("/login", async (req, res) => {
  try {
    ...
    console.log(
      "[LOGIN] Input password (first 2 chars):",
...
${finalAppHash}`;
      ...
      console.log(
        `[CHECK-PHONE] ✅ Fast2SMS Response:`,
        JSON.stringify(fast2smsResponse.data, null, 2),
      );

      if (!fast2smsResponse.data || fast2smsResponse.data.return === false) {
        console.error(
          `[CHECK-PHONE] ❌ Fast2SMS API returned error:`,
        ...
        return res.json({
          ...
          _debug:
            process.env.NODE_ENV === "development"
              ? { otp, error: fast2smsResponse.data }
              : undefined,
        });
      }
...
router.post("/verify-otp", async (req, res) => {
  try {
    ...
    if (!verificationResult.success) {
      console.log(
        `[VERIFY-OTP] ❌ ${verificationResult.message} for ${normalizedPhone}`,
      );
      return res.status(400).json({
        success: false,
...
router.post("/reset-password", async (req, res) => {
  try {
    ...
    console.log(
      "[RESET-PASSWORD] New password (first 2 chars):",
      newPassword?.substring(0, 2),
    );
    ...
    console.log(
      "[RESET-PASSWORD] Searching for user with phone variants:",
      phoneVariants,
    );
...
router.post("/update-service-type", requireAuth, async (req: AuthReq, res) => {
  try {
    ...
    console.log(
      `📝 [UPDATE-SERVICE-TYPE] Request received for user: ${req.userId}`,
    );
    ...
    if (
      ...
      console.log(
        `❌ [UPDATE-SERVICE-TYPE] Invalid service type: ${serviceType}`,
      );
      return res.status(400).json({
        success: false,
        message:
    ...
    console.log(
      `✅ [UPDATE-SERVICE-TYPE] Service type updated successfully for user: ${req.userId}`,
    );
